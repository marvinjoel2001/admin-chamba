...
export default function DashboardPage() {
  ...
  return (
    <div className="relative min-h-[calc(100vh-120px)] w-full overflow-hidden rounded-3xl border border-white/10 dark:border-white/5 bg-gradient-to-br from-white/90 via-purple-50/40 to-white/95 dark:from-[#0a0514] dark:via-[#130d24] dark:to-[#0a0514] shadow-2xl flex flex-col items-center justify-center p-8 transition-all duration-300">
      ...
      {/* Curved Horizon at bottom (dark mode) */}
      <div className="absolute -bottom-32 left-1/2 -translate-x-1/2 w-[150%] h-[300px] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-purple-600/10 via-purple-900/10 to-transparent blur-3xl rounded-[100%] pointer-events-none" />

      {/* Top right widgets (Date & Time) */}
      <div className="absolute top-6 right-6 hidden sm:flex items-center gap-4 bg-purple-500/5 dark:bg-white/5 border border-purple-500/15 dark:border-white/10 rounded-full px-4 py-2 text-xs text-purple-900 dark:text-purple-200/70 backdrop-blur-md z-20 shadow-sm">
        <div className="flex items-center gap-2 font-medium">
          <CalendarDays className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400" />
          <span>{dateString}</span>
        </div>
        <div className="w-1 h-1 rounded-full bg-purple-500/30 dark:bg-white/20" />
      ...
      <div className="relative z-10 flex flex-col items-center">
        {/* Icon */}
        <div className="w-16 h-16 rounded-2xl bg-purple-500/15 dark:bg-purple-900/40 border border-purple-500/30 flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(168,85,247,0.2)]">
          <Users className="w-8 h-8 text-purple-600 dark:text-purple-300" />
        </div>

        <h2 className="text-purple-900/70 dark:text-purple-200/80 text-xs md:text-sm font-bold tracking-[0.2em] uppercase mb-4 text-center">
          Usuarios Registrados en la App
        </h2>

        <div className="relative my-2">
          <h1 className="text-7xl sm:text-8xl md:text-9xl font-black text-gray-950 dark:text-white tracking-tighter drop-shadow-sm dark:drop-shadow-[0_0_40px_rgba(168,85,247,0.6)]">
            {loading ? "..." : totalUsers.toLocaleString("en-US")}
          </h1>
        </div>

        <p className="text-gray-500 dark:text-purple-200/50 text-sm mt-2 font-medium">
          Total de usuarios registrados
        </p>

        <div className="mt-8 bg-purple-600/10 dark:bg-purple-900/40 border border-purple-500/30 dark:border-purple-500/40 rounded-full px-6 py-2.5 flex items-center gap-3 backdrop-blur-md shadow-sm dark:shadow-[0_0_20px_rgba(168,85,247,0.2)] hover:bg-purple-600/15 dark:hover:bg-purple-800/50 transition-colors cursor-default">
          <Users className="w-4 h-4 text-purple-600 dark:text-purple-300" />
          <span className="text-purple-900 dark:text-purple-100 font-bold text-sm">+{newUsersToday} hoy</span>
        </div>
      </div>
    </div>
  );
